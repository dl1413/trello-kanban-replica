...
class TestContinuousControlEnv:
    ...
    def test_velocity_clamping(self):
        ...
        max_vel = 2.0
        env = ContinuousControlEnv(
            {"dt": 0.1, "friction": 0.0, "mass": 0.1, "max_velocity": max_vel}  # Low mass for high acceleration
        )
        env.reset(seed=42)
    ...
    def test_goal_termination(self):
        ...
        env = ContinuousControlEnv({"goal_threshold": 0.5, "episode_length": 1000})
        env.reset(seed=42)
    ...
    def test_observation_normalization(self):
        ...
        env = ContinuousControlEnv({"normalize_obs": True, "workspace_size": 10.0, "max_velocity": 5.0})
        obs, info = env.reset(seed=42)
