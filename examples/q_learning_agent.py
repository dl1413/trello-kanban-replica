...
class QLearningAgent:
    ...
    def update(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray, terminated: bool):
        """
...
def evaluate_agent(env: SimpleGridWorld, agent: QLearningAgent, num_episodes: int = 100) -> Dict[str, float]:
    """
...
def plot_training_results(stats: Dict[str, Any], save_path: str = None):
    ...
    if len(stats["episode_rewards"]) >= window:
        ...
        ax.plot(
            range(window - 1, len(stats["episode_rewards"])),
            moving_avg,
    ...
    if len(stats["episode_lengths"]) >= window:
        ...
        ax.plot(
            range(window - 1, len(stats["episode_lengths"])),
            moving_avg,
            label=f"{window}-Episode Moving Avg",
            linewidth=2,
        )
...
def main():
    ...
    env_config = {"grid_size": 5, "episode_length": 100, "reward_type": "sparse"}  # or 'dense' for shaped rewards
    ...
    stats = train_q_learning(
        env=env, agent=agent, num_episodes=num_episodes, max_steps=100, eval_interval=50, verbose=True
    )
    ...
    improvement = (
        (final_eval["mean_reward"] - random_eval["mean_reward"]) / abs(random_eval["mean_reward"]) * 100
        if random_eval["mean_reward"] != 0
        else float("inf")
    )
