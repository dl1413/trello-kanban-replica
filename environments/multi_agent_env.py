...
class MultiAgentBaseEnvironment(BaseEnvironment):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.observation_spaces = {
            i: spaces.Box(low=0, high=255, shape=(84, 84, 3), dtype=np.uint8) for i in range(self.num_agents)
        }
    ...
    def _calculate_rewards(
        self, actions: Dict[int, int], prev_terminated: Optional[Dict[int, bool]] = None
    ) -> Dict[int, float]:
        """
    ...
    def _get_infos(self) -> Dict[int, Dict[str, Any]]:
        ...
        """
        return {
            i: {"step": self.current_step, "episode_length": self.episode_length, "agent_id": i}
...
class MultiAgentGridWorld(MultiAgentBaseEnvironment):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        for i in range(self.num_agents):
            ...
            self.observation_spaces[i] = spaces.Box(low=0, high=self.grid_size - 1, shape=(2,), dtype=np.int32)
    ...
    def _get_initial_states(self) -> Dict[int, np.ndarray]:
        ...
        for i in range(self.num_agents):
            while True:
                pos = np.array(
                    [self.np_random.integers(0, self.grid_size), self.np_random.integers(0, self.grid_size)],
                    dtype=np.int32,
                )
                pos_tuple = tuple(pos)
    ...
    def _calculate_rewards(
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.collision_mode == "penalty":
                ...
                for other_id in range(self.num_agents):
                    if other_id != agent_id and np.array_equal(
                        self.agent_positions[agent_id], self.agent_positions[other_id]
                    ):
        ...
        if self.reward_mode == "cooperative":
            all_reached = all(
                np.array_equal(self.agent_positions[i], self.goal_positions[i]) for i in range(self.num_agents)
            )
            if all_reached:
                for agent_id in range(self.num_agents):
    ...
    def render(self, mode: str = "human"):
        ...
        if mode == "human":
            ...
            for agent_id in range(self.num_agents):
                ...
                print(
                    f"Agent {agent_id}: pos={info['agent_pos']}, dist={info['distance_to_goal']:.2f}, goal={info['goal_reached']}"
                )
