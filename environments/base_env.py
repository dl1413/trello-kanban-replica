...
class BaseEnvironment(gym.Env):
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        return {"step": self.current_step, "episode_length": self.episode_length}

    def _get_rgb_array(self) -> np.ndarray:
        """Get RGB array for rendering. Override in subclass."""
        return self.state if self.state is not None else np.zeros(self.observation_space.shape, dtype=np.uint8)
